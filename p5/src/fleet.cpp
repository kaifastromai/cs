...
void Fleet::Step()
{
    ...
    bool is_empty = lcl_rockets.empty();
...
{
    Rocket_Type type = Rocket_Type(utils::unif<int>(0, 2));
    ...
    case IE_Rocket:
    {
        ...
        r->SetAgeLimit(utils::Jiggle(20, 40));
        r->SetPosition(utils::Jiggle(COLS / 2, 50), 0);
        return r;
        break;
    }
    case IE_Palm_Tree:
    {
        ...
        p->SetAgeLimit((std::get<0>(forcev)) / -g);
        p->SetPosition(utils::Jiggle<int>(COLS / 2, 100), 0);
        p->SetTriggerAge((std::get<0>(forcev)) / 0.2);
    ...
    case IE_Streamer:
    {
        ...
        s->SetAgeLimit((std::get<0>(forcev)) / -g);
        s->SetPosition(utils::Jiggle<int>(COLS / 2, 100), 0);
        s->SetTriggerAge((std::get<1>(forcev)) / 0.2);
        return s;
        break;
    }
