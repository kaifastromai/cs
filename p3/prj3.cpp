...
int main(int argc, char *argv[])
{
    ...
    if (argc > 2)
    {
        //Should set local for std::cout (properly formatting numbers...etc)
        ...
        {
            cout << "This is not a valid company file! Maybe you switched the order?" << endl;
            return 1;
        };
        //same with the data.csv
        if (!purchase::get_purchases(idToTransactions, argv[2]))
        {
            cout << "This is not a valid data file! Maybe you switched the order?" << endl;
            return 1;
        }
